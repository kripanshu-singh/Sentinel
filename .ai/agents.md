...
The entire project lives in `.ai/`. Read these before any non-trivial change:

| # | File | What it gives you |
|---|------|-------------------|
| 1 | `.ai/project_overview.md` | What Sentinel is, who it's for, the core capabilities (the walkthrough), and non-goals. |
| 2 | `.ai/architecture.md` | **Why** the frontend/worker split exists, the agent state machine, the streaming channels, and where every file belongs. |
| 3 | `.ai/ui_context.md` | **The most important UI file.** Design system hard rules + the exact spec for the goal input, live run (incl. the HITL modal), and result screens. |
| 4 | `.ai/code_standards.md` | **The most important code file.** Non-negotiable rules (no inline CSS, Tailwind + semantic tokens, Zod at boundaries, React Query, composition, server-first) and the verification checklist. |
| 5 | `.ai/decisions.md` | Every ADR (frontend/worker split, HITL protocol, streaming, LLM, storage, Zod, React Query). Constraints you must respect. |
| 6 | `.ai/acceptance_criteria.md` | Each feature as checkable `[ ]` criteria — verify against these before reporting done. |
| 7 | `.ai/prompts.md` | Reusable copy/paste prompts for common tasks (build a screen, add a component, etc.). |
| 8 | `.ai/roadmap.md` | Current phase and what's planned next. |
...
- **Never guess project specifics** — the `.ai/` files above are canonical. If a requirement
...
- **Verify before reporting done.** Run `npm run lint` and `npm run build`, then confirm against
  the relevant `.ai/acceptance_criteria.md` boxes.
...
1. Read the `.ai/*.md` context above (in order).
2. Explore `src/` to find the right home for the change (`.ai/architecture.md` → module map).
3. Implement per `.ai/code_standards.md` and `.ai/ui_context.md`.
4. Run `npm run lint` and `npm run build` (and tests if present).
5. Verify against the matching `.ai/acceptance_criteria.md` criteria.
