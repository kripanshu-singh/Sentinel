...
- **Consequences:** Anonymous visitors get **5 runs/cookie/UTC day** (accounts are deferred, so this is the product allowance for everyone; no account funnel). Cookie-reset abuse is bounded by the per-IP backstop (100/day) and, critically, by the global capacity ceiling (`SENTINEL_GLOBAL_ACTIVE_LIMIT`, default 1 for the 512MB Render free worker), which caps maximum cost regardless of identity rotation. The UI shows the local-time daily reset and always defers to the server's 429 + snapshot.
